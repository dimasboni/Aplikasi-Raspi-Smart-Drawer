...
def register_admin_pages(page: ft.Page, session_data: dict, nav: dict):
    ...
    def show_manage_tools_page(e=None):
        ...
        def buka_dialog_edit(nama_alat_lama, rfid_lama, gambar_lama, kondisi_lama):
            ...
            def eksekusi_simpan(e):
                try:
                    ...
                    if os.path.isabs(filepath_asli):
                        ...
                        if os.path.abspath(filepath_asli) != os.path.abspath(
                            lokasi_simpan
                        ):
                            ...
                            if os.path.exists(lokasi_simpan):
                                ...
                                while os.path.exists(
                                    os.path.join(
                                        "assets", f"{nama_file}_{counter}{ext}"
                                    )
                                ):
                                    counter += 1
                            ...
                            lokasi_simpan_baru = (
                                lokasi_simpan  # Menandai bahwa gambar di-Update
                            )
                            ...
                            try:
                                ...
                                if lokasi_simpan_baru:
                                    with open(lokasi_simpan_baru, "rb") as f:
                                        payload["gambar_base64"] = base64.b64encode(
                                            f.read()
                                        ).decode("utf-8")
            ...
            kolom_kiri_edit = ft.Column(
                [input_nama, input_rfid, dd_kondisi_edit],
                width=250,
                height=300,
            )
                    ...
                    content=ft.Row(
                        [
                            ...
                            ft.Container(
                                ...
                                padding=10,
                                on_click=lambda _, n=nama_alat, r=rfid_alat: konfirmasi_hapus(
                                    n, r
                                ),
                                ink=True,
                            ),
        ...
        tombol_laci = ft.SegmentedButton(
            on_change=lambda e: muat_daftar_alat(
                input_search.value, list(e.control.selected)[0]
            ),
        ...
        input_search = ft.TextField(
            ...
            color="black",
            on_change=lambda e: muat_daftar_alat(
                e.control.value, list(tombol_laci.selected)[0]
            ),
        )
    ...
    def show_history_page(e=None):
        ...
        try:
                ...
                rows = [
                    ft.DataRow(
                        cells=[
                            ft.DataCell(
                                ft.Text(
                                    str(row[0]), color="black", weight="bold", size=15
                                )
                            ),
                            ft.DataCell(
                                ft.Text(
                                    str(row[1]), weight="bold", color="black", size=15
                                )
                            ),
                            ft.DataCell(
                                ft.Text(
            ...
            table = ft.DataTable(
                columns=[
                    ft.DataColumn(
                        ft.Text("User", weight="bold", color="black", size=20)
                    ),
                    ft.DataColumn(
                        ft.Text("Alat", weight="bold", color="black", size=20)
                    ),
                    ft.DataColumn(
                        ft.Text("Waktu", weight="bold", color="black", size=20)
                    ),
                    ft.DataColumn(
                        ft.Text("Status", weight="bold", color="black", size=20)
                    ),
                ],
                rows=rows,
                border=ft.border.all(1, "#E5E7EB"),
    ...
    def show_add_tool_page(e=None):
        ...
        dd_pin = ft.Dropdown(
            ...
            options=[
                ft.dropdown.Option(
                    key=f"P{str(i).zfill(2)}", text=f"P{str(i).zfill(2)}"
                )
                for i in range(1, default_slot + 1)
            ],
        )
        ...
        def update_pin_options(e):
            ...
            try:
                with sqlite3.connect("smartdrawer.db", timeout=20) as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                        "SELECT mqtt_topic FROM tools WHERE page =?", (laci_terpilih,)
                    )
                    hasil = cursor.fetchall()
                    # hasilnya berupa list of tuples yang akan mengambil list posisi
                    pin_terpakai = [
                        str(baris[0]).strip() for baris in hasil if baris[0]
                    ]
            ...
            if len(dd_pin.options) == 0:
                dd_pin.options.append(
                    ft.dropdown.Option(
                        key="", text="Position is not available", disabled=True
                    )
                )
        ...
        def simpan_alat_baru(e):
            bunyikan_buzzer_error(
                0.5
            )  # Bunyi buzzer error sebagai pancingan untuk segera meletakkan alat di sensor, bisa disesuaikan durasinya
            ...
            try:
                with sqlite3.connect("smartdrawer.db", timeout=20) as conn:
                    cursor = conn.cursor()
                    res = cursor.execute(
                        "SELECT name FROM tools WHERE rfid_tag_uid = ?", (uid_tag,)
                    ).fetchone()
                    if res:
                        notif_text.value = (
                            f"❌ Tag RFID sudah terdaftar untuk alat '{res[0]}'!"
                        )
            ...
            teks_laci_tambah = ft.Text(
                f"Drawer {dd_laci.value} Open", weight="bold", color="blue", size=18
            )
            teks_posisi_tambah = ft.Text(
                f"Please place the Tool in Position {dd_pin.value}"
            )
            ...
            dialog_tunggu_sensor = ft.AlertDialog(
                ...
                content=ft.Column(
                    [
                        ...
                        teks_posisi_tambah,
                        ft.Text(
                            "System will save automatically when the tool is placed",
                            color="grey",
                            size=12,
                            text_align="center",
                        ),
            ...
            async def pantau_sensor_tambah():
                ...
                while status_sensor_realtime.get(kunci_unik, 0) == 0:
                    if target_expected.get("lockdown"):
                        ...
                        teks_posisi_tambah.value = (
                            f"Please place the Tool in Position {pin_terpilih}"
                        )
                        ...
                        def kirim_api_tambah():
                            try:
                                with open(lokasi_simpan, "rb") as f:
                                    gambar_b64 = base64.b64encode(f.read()).decode(
                                        "utf-8"
                                    )
                                ...
                                if response.status_code in [200, 201]:
                                    ...
                                    print(
                                        f"❌ API Menolak! Status: {response.status_code} | Jawaban: {response.text}"
                                    )

                            except Exception as e:
                                print(
                                    f"❌ Gagal Terkoneksi (Server Mati/Jaringan Terputus): {e}"
                                )
    ...
    def show_sync_web_page(e=None):
        ...
        dialog_scan = ft.AlertDialog(
            ...
            content=ft.Column(
                [ft.Text("Scan tag untuk alat ini:"), input_popup_scan, notif_text],
                tight=True,
            ),
        )
        ...
        def muat_antrean_pending():
            ...
            try:
                ...
                response = requests.get(
                    f"http://{ip_server}/api/v1/alat-pending", timeout=10
                )
                ...
                if not alat_pending:
                    list_ui.controls.append(
                        ft.Container(
                            content=ft.Text(
                                "✅ Tidak ada antrean alat pending dari Web!",
                                color="green",
                                weight="bold",
                            ),
                            padding=20,
                ...
                laci_terkecil_aktif = min(
                    [int(alat["laci_id"]) for alat in alat_pending]
                )
                ...
                for alat in alat_pending:
                    ...
                    kotak = ft.Container(
                        content=ft.Row(
                            [
                                ft.Container(
                                    content=ft.Text(f"Laci {laci_alat}", weight="bold"),
                                    width=60,
                                ),
                ...
                list_ui.controls.append(
                    ft.Text(f"Gagal koneksi ke server: {e}", color="red")
                )
            page.update()
        ...
        def proses_rfid_submit(e):
            ...
            try:
                with sqlite3.connect("smartdrawer.db", timeout=20) as conn:
                    if (
                        conn.cursor()
                        .execute(
                            "SELECT name FROM tools WHERE rfid_tag_uid = ?", (uid,)
                        )
                        .fetchone()
                    ):
            ...
            teks_posisi_tambah.value = (
                f"Taruh {alat_terpilih['kode_alat']} di Pin {pin_target}"
            )
            dialog_tunggu_sensor.open = True
            page.update()
        ...
        async def pantau_sensor(laci_target, pin_target, uid_tag):
            ...
            if status_sensor_realtime.get(kunci_unik, 0) == 1:
                try:
                    ...
                    def kirim_konfirmasi():
                        try:
                            ...
                            requests.post(
                                url, json={"uid_tag_rfid": uid_tag}, timeout=10
                            )
                        except Exception:
                            pass
                    ...
                    page.snack_bar = ft.SnackBar(
                        ft.Text("✅ Alat sukses disinkronkan!"),
                        bgcolor="green",
                        open=True,
                    )
                    page.update()
                ...
                dialog_tunggu_sensor.open = False
                page.snack_bar = ft.SnackBar(
                    ft.Text("❌ Timeout: Sensor tidak mendeteksi alat."),
                    bgcolor="red",
                    open=True,
                )
                page.update()
        ...
        main_card = ft.Container(
            content=ft.Column(
                [ft.Text("Antrean Alat dari Website", size=24, weight="bold"), list_ui],
                horizontal_alignment="center",
                spacing=15,
            ),
            width=700,
