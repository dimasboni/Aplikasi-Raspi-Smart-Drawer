    ...
    return ft.Container(
        content=ft.Column(
            [
                ft.Container(
                    content=ft.Image(
                        src=f"/{item_data['img']}", height=90, fit="contain"
                    ),
                    rotate=ft.Rotate(
                        angle=get_rotation_angle(rotation_deg),
                ...
                ft.Text(
                    ...
                    size=14,
                    weight="bold",
                    color="#B0BEC5" if is_empty else TEXT_COLOR,
                    text_align="center",
                    max_lines=2,
                ),
                ft.Text(
                    "HABIS" if is_empty else f"Stok: {stok}",
                    size=12,
                    weight="bold",
                    color="red" if is_empty else "green",
                ),
            ],
            alignment="center",
            horizontal_alignment="center",
        ...
        on_click=lambda e: (
            on_click(item_data["name"], item_data) if not is_empty else None
        ),
        disabled=is_empty,
        ink=not is_empty,
        alignment=ft.Alignment(0, 0),
    ...
    header_row = ft.Row(
        [
            (
                ft.Container(
                    content=ft.Text(
                        "⬅️ Kembali", size=18, weight="bold", color=TEXT_COLOR
                    ),
                    on_click=back_func,
                    padding=10,
                    border_radius=10,
