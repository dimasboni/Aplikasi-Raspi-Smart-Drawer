    ...
    def show_sync_web_page(e=None):
        ...
        def muat_antrean_pending():
            ...
            try:
                ...
                print("=" * 50)
                print("ADMIN LOGIN :", admin_username)

                response = requests.get(
                    f"http://{ip_server}/api/v1/alat-pending",
                ...
                print("ADMIN LOGIN :", admin_username)
                print("DATA DARI WEB :", alat_pending)
                ...
                alat_siap_masuk = [
                    ...
                    if alat.get('page') and alat.get('mqtt_topic') is not None
                ]
                ...
                if not alat_siap_masuk:
                    list_ui.controls.append(
                        ft.Container(
                            content=ft.Text(
                                "Alat pending di web, laci/pin belum diatur!",
                                color="orange",
                                weight="bold"
                            ),
                            padding=20
                ...
                alat_siap_masuk.sort(
                    key=lambda x: (int(x['page']), x['mqtt_topic'])
                )

                laci_terkecil_aktif = min(
                    int(alat['page'])
                    for alat in alat_siap_masuk
                )

                for alat in alat_siap_masuk:
                    laci_alat = int(alat['page'])
        ...
        def proses_rfid_submit(e):
            ...
            dialog_scan.open = False
            laci_target = int(alat_terpilih["page"])
        ...
        async def pantau_sensor(laci_target, pin_target, uid_tag):
            ...
            if status_sensor_realtime.get(kunci_unik, 0) == 1:
                try:
                    print("=== DEBUG INSERT SQLITE ===")
                    print("Nama :", alat_terpilih['kode_alat'])
                    print("RFID :", uid_tag)
                    print("Foto :", alat_terpilih['foto'])
                    print("Page :", laci_target)
                    print("Pin  :", pin_target)
                    print("Kondisi :", alat_terpilih['kondisi'])
                    ...
                    if user_record: 
                        ...
                        if db_password.startswith("$2"):
                            ...
                            if bcrypt.checkpw(input_password.encode('utf-8'), db_password.encode('utf-8')):
                                session_data["admin_username"] = username_field.value.strip()
